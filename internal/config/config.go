...
type Config struct {
	...
	// Default: /tmp/animelistgen.lastrun
...
const (
	...
	DefaultDescriptionTemplate = "All anime TV series airing in {season} {year}, sourced from AniList. Generated by animelistgen."
	// DefaultInterval is the default daemon interval (1 week).
	DefaultInterval = 168 * time.Hour
	// DefaultStateFile is where the last-run timestamp is written.
	DefaultStateFile = "/tmp/animelistgen.lastrun"
...
func searchPaths(cliPath string) []string {
	...
	paths := []string{filepath.Join(".", "animelistgen.yaml")}
	...
	if xdg != "" {
		paths = append(paths, filepath.Join(xdg, "animelistgen", "animelistgen.yaml"))
	}
...
func WriteDefaultConfig(path string) error {
	content := `# animelistgen configuration
# See https://github.com/calmcacil/animelistgen for full documentation.
...
mdblist:
  ...
  # Default: "All anime TV series airing in {season} {year}, sourced from AniList. Generated by animelistgen."
...
# Default: /tmp/animelistgen.lastrun
state_file: /tmp/animelistgen.lastrun
`
...
func ResolveConfigPath(cliPath string) string {
	...
	return filepath.Join(xdg, "animelistgen", "animelistgen.yaml")
}
